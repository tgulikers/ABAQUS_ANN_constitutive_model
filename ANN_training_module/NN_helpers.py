...
def NNpredict(model, dataArr_in, normalisationParams, keys_in, keys_inc, keys_out):
    ...
    for i_key in range(len(damageKeyLst)):
        key = damageKeyLst[i_key].strip('DamageParameter_')
        ratio_in    = dataArr_out['Epsilon_' + key] / dataArr_out['Sigma_' + key]
        ratio_out   = (dataArr_out['Epsilon_' + key] + delta['dEpsilon_' + key]) / \
                      (dataArr_out['Sigma_' + key] + delta['dSigma_' + key])
...
def createUMAT(fname, matname, model, normalisationParams, n_dim, solver = 'secant'):
    ...
    if W.shape[0] >= 5:
        f.write('  ! fourth hidden layer calculation\n' +
                ...
                '  deallocate(w_ED)\n'+
                '  deallocate(b_ED)\n\n')
    f.write('  ! output layer assignment of variables\n' +
            '  deallocate (net_input)\n' +
            '  deallocate (x)\n' +
